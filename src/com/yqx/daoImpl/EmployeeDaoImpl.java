...
public class EmployeeDaoImpl implements EmployeeDao{
	...
	public List<EmployeeVO> getByPage(int currentPage, int pageSize) {
		List<EmployeeVO> list = new ArrayList<>();
		Connection con = DBUtil.getConnection();
		PreparedStatement pst = null;
		ResultSet rs = null;
		String sql = "select * from hr_employee,hr_department where hr_employee.deptId=hr_department.id order by hr_employee.id limit ?,?";
		try {
			pst = con.prepareStatement(sql);
			pst.setInt(1, (currentPage - 1) * pageSize);
			pst.setInt(2, pageSize);
			...
			while (rs.next()) {
				...
				e.setId(rs.getInt("hr_employee.id"));
				e.setName(rs.getString("hr_employee.name"));
				e.setSex(rs.getInt("hr_employee.sex"));
				e.setAge(rs.getInt("hr_employee.age"));
				e.setEdu(rs.getInt("hr_employee.edu"));
				e.setDegree(rs.getInt("hr_employee.degree"));
				e.setJob(rs.getString("hr_employee.job"));
				e.setDeptname(rs.getString("hr_department.name"));
				e.setState(rs.getInt("hr_employee.state"));
				e.setPhone(rs.getString("hr_employee.phone"));
				e.setAddress(rs.getString("hr_employee.address"));
				e.setCreateTime(rs.getTimestamp("hr_employee.createTime"));
				list.add(e);
			}
		} catch (SQLException e) {
			e.printStackTrace();
